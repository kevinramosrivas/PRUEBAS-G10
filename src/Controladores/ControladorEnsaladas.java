 ...
 * @author Lenovo
 */
public class ControladorEnsaladas implements ActionListener{

    private ListaDeDatos modelo;
    private VistaEnsaladas vista;

    public ControladorEnsaladas(ListaDeDatos modelo, VistaEnsaladas vista) throws Exception {
        this.modelo = modelo;
        this.vista = vista;
    ...
    public class ClickTablaEnsaladas implements MouseListener
    {
        ...
        public void mouseClicked(MouseEvent ev) {
            ...
            int column = vista.tblEnsaladas.getColumnModel().getColumnIndexAtX(ev.getX());
            int row = ev.getY() / vista.tblEnsaladas.getRowHeight();

            if(row < vista.tblEnsaladas.getRowCount() && row >= 0 &&
                column < vista.tblEnsaladas.getColumnCount() && column>=0)
            {

                Object value  = vista.tblEnsaladas.getValueAt(row, column);
                if(value instanceof JButton)
                {
                    ((JButton)value).doClick();
                    JButton boton = (JButton) value;
                    try {
                        agregarEnsaladaTablaOrden(row, column , "ensaladas");
                    } catch (Exception ex) {
                        Logger.getLogger(ControladorEnsaladas.class.getName()).log(Level.SEVERE, null, ex);
                    }
        ...
        public void mouseClicked(MouseEvent ev) {
                ...
                column < vista.tblNuevaOrden.getColumnCount() && column>=0)
            {
                ...
                if(value instanceof JButton)
                {
                    ...
                    {
                        try {
                            agregarEnsaladaTablaOrden(row, column, "orden");
                        } catch (Exception ex) {
                            Logger.getLogger(ControladorEnsaladas.class.getName()).log(Level.SEVERE, null, ex);
                        }
                    }
    ...
    public void actionPerformed(ActionEvent ev) {
        ...
        if(ev.getActionCommand().equals(InterfazEnsaladas.INICIO))
        {
        ...
        {
            if(ev.getActionCommand().equals(InterfazEnsaladas.VerYModificarOrden))
            {
                try
                {
                    vista.setVisible(false);
            ...
            {
                if(ev.getActionCommand().equals(InterfazEnsaladas.PAGARORDEN))
                {
                    try {
                ...
                {
                    if(ev.getActionCommand().equals(InterfazEnsaladas.ATRAS))
                    {
                        vista.setVisible(false);
                        VistaMenuInicio vistaInicio = new VistaMenuInicio();
                        ControladorMenuInicio2 control = new ControladorMenuInicio2(modelo,vistaInicio);
                    ...
                    {
                        if(ev.getActionCommand().equals(InterfazEnsaladas.MICOMPRA))
                        {
    ...
    public void llenarTablaEnsaladas()
    {
        ListaEs.Nodo nodo = modelo.listaEnsaladas.padre;
        DefaultTableModel dtm = (DefaultTableModel)vista.tblEnsaladas.getModel();
        int nfila=0,ncol=0;
        while(nodo!=null)
        {
            Ensalada bd = (Ensalada)nodo.elemento;
        ...
        vista.tblEnsaladas.setDefaultRenderer(Object.class, new Render());
    ...
    public void llenarTablaOrdenEnsaladas() throws Exception
    {
        limpiarTablaOrden();
    ...
    public void agregarEnsaladaTablaOrden(int row,int column,String tabla) throws Exception
    {
        DefaultTableModel dtmOrden = (DefaultTableModel)vista.tblNuevaOrden.getModel();
        DefaultTableModel dtmEnsaladas = (DefaultTableModel)vista.tblEnsaladas.getModel();
        Producto producto;
        if(tabla.equals("ensaladas"))
        {
            String nombre = (String)dtmEnsaladas.getValueAt(row, 0);
            producto = devolverEnsalada(nombre);
        }
        else
        {
        ...
        else
        {
            ...
            (modelo.cliente.getOrden(), producto.getNombre(), producto.getPrecio(), 
                    DaoFactory.getinstance().getCategoriaDao("mysql").devolverCategoria("Ensaladas"), 1);
        }
        llenarTablaOrdenEnsaladas();
    }
    ...
    public Ensalada devolverEnsalada(String nombreEnsalada)
    {
        ...
        while(nodo!=null)
        {
            ...
            if(auxEnsalada.getNombre().equals(nombreEnsalada))
            {
